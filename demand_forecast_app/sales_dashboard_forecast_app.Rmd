...
source("scripts/04_demand_forecast.R")
```
...
renderUI({
  ...
  if( input$view_mode) {
    ...
    # includeCSS() programmatically includes css in your app
...
h4("Forecast Mode")
# switchinput() toggle switch that comes in various styles
switchInput(
...
# conditionalPanel() is a panel that is hidden until a CSS element ID toggles it on
conditionalPanel(
...
shinyWidgets::radioGroupButtons(
  ...
  selected = "month", 
  # Note, changing from primary to success for easier color change in css.
  status   = "success", 
  justified = TRUE, 
  checkIcon = list(
    yes = icon("ok", lib = "glyphicon"), 
...
# ObserveEvent() watches a button, and fire code when an action (click) takes place
observeEvent(eventExpr = input$time_unit, {
  
  if (input$forecast_mode) {
    # click() simulate a clicking action with JavaScript
...
# Pro Tip: If you want a code to only fire the first time, set once = TRUE.  This prevents the
# forecast from firing every time the user toggles when it only needs to happen the 1st time.
